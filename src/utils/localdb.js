...
export const readMaterials = async (filters = {}) => {
  ...
  // If a backend server is available on the same origin, try fetching materials from it
    try {
      ...
      return await api.apiGet(`/api/materials?${params.toString()}`);
    } catch (e) {
      console.warn('Failed to fetch materials from backend via apiGet, falling back to localStorage', e && e.message);
    }
